...
This module provides functionality to interact with OpenAI's ChatCompletion API.
It includes functions to configure the OpenAI API, call the API with a prompt, and run an
interactive shell interface similar to the local mode, but using OpenAI's models.
...
def call_openai(command: str, openai_model: str, retry: int) -> str:
    """
    Use the OpenAI API to translate a natural language command into a shell command.
    ...
    Returns:
        str: The extracted shell command. If proper formatting is not achieved, returns raw output.
    """
    shell = detect_shell()
    ...
    while True:
        try:
            ...
            if user_input.startswith("!"):
                ...
                # Allow user to edit generated command
